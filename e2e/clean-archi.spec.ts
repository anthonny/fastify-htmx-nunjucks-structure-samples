...
test.describe("Clean architecture", () => {
	test("should display users page with initial users list", async ({
		page,
	}) => {
		await page.goto("/clean-archi/users");

		// Check if the page loaded
		await expect(page.locator("body")).toBeVisible();

		// Check if shuffle button is present
		const shuffleButton = page.locator('button:has-text("Shuffle users")');
	...
	test("should return JSON when Accept header is application/json", async ({
		...
		const response = await request.get("/clean-archi/users", {
			headers: {
